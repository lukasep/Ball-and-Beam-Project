...
typedef RT_MODEL_simulink_experiment__T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * simulink_experiment_test_type1(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(simulink_experiment_test_typ_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(simulink_experiment_test_typ_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://simulink_experiment_test_type1:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: simulink_experiment_test_type1 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = simulink_experiment_test_type1();
  if (rtmGetErrorStatus(S) != NULL) {
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (simulink_experiment_test_typ_DW.HILReadEncoderTimebase_Task, (t_clock)
         simulink_experiment_test_type_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
