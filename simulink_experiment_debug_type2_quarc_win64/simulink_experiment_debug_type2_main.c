 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * simulink_experiment_debug_type2(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(simulink_experiment_debug_ty_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://simulink_experiment_debug_type2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: simulink_experiment_debug_type2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = simulink_experiment_debug_type2();
  if (rtmGetErrorStatus(S) != NULL) {
